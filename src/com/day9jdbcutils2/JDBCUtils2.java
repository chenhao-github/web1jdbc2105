...
//            dataSource.setInitialPoolSize(20);//初始创建20个线程
//            dataSource.setMinPoolSize(2);//至少连接2个
//            dataSource.setMaxPoolSize(50);//最大连接50个
            System.out.println("连接池设置完成1");
...
//        }
    ...
    public static Connection getConn(){
        try {
            conn = dataSource.getConnection();//通过连接池中获取一个连接使用
        ...
        try {
            ...
            try {
                if(ps != null)
                    ps.close();
                if(conn != null)
                    conn.close();//关闭连接池中连接的使用，返回给连接池供其他任务重复使用
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
        }
